            ...
            <div class="flex flex-col gap-4 p-6 border-r border-stone-200 dark:border-stone-800 overflow-y-auto">
                ...
                <UFormField label="Short description">
                    <UTextarea v-model="form.description"
                        placeholder="A short story about a forgotten house on the edge of the city." :rows="2"
                ...
                <UFormField label="Story text">
                    <UTextarea v-model="form.text" placeholder="Paste the full story here…" :rows="12"
            ...
            <div class="flex flex-col gap-4 p-6 bg-stone-100 dark:bg-stone-900 overflow-y-auto">
                ...
                <template v-else>
                    ...
                    <div
                        ...
                        <p class="text-sm leading-loose text-stone-700 dark:text-stone-300">
                            {{ words.join(' ') }}
                        </p>
                    </div>
                </template>
                ...
                <div class="flex flex-col gap-3">
                    ...
                    <UFormField label="Example sentence">
                        <UInput v-model="dictEntry.example" placeholder="She lived on the outskirts of the city."
                            class="w-full" @keydown.enter="addEntry" />
                    </UFormField>
...
function processStory() {
    ...
    setTimeout(() => {
        const tokens = form.text.trim().match(/[\w']+|[^\w\s]/g) ?? []
        words.value = tokens
...
function removeEntry(i) {
    dictionary.value.splice(i, 1)
}

function focusTranslation() {
    nextTick(() => translationInput.value?.input?.focus())
}

function focusExample() {
    nextTick(() => exampleInput.value?.input?.focus())
}
